...
class DenoisingNet_MLP(torch.nn.Module):
    def __init__(
        ...
        self.linear4 = torch.nn.Linear(H_3, H_4, bias=True) # adding an extra hidden layer for the original Deep KSVD

        self.linear5 = torch.nn.Linear(H_4, D_out_lam, bias=True)
    ...
    def forward(self, x):
        ...
        lin = self.linear4(lin).clamp(min=0)
        lam = self.linear5(lin)
